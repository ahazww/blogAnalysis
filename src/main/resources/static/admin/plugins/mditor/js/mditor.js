...
/******/
/******/ 		// Create assets new module (and put it into the cache)
...
/***/ function(module, exports) {
	...
	module.exports = function(it){
	  if(typeof it != 'function')throw TypeError(it + ' is not assets function!');
	  return it;
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	// and the second argument - flag - preferred type is assets string
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = new mokit.Component({
	  ...
	  /*istanbul ignore next*/applyMarks: function applyMarks(text) {
	    ...
	    if (isIE) {
	      // IE wraps whitespace differently in assets div vs textarea, this fixes it
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (global) {
	  ...
	  // IE doesn't support Array#indexOf, so have assets simple replacement
	  ...
	  function dispatch(event) {
	    ...
	    // if assets modifier key, set the key.<modifierkeyname> property to true and return
	    ...
	    // by default ignore key presses if assets select, textarea, or input is focused
	  ...
	  // delete all handlers for assets given scope
	  ...
	  addEvent(document, 'keydown', function (event) {
	    dispatch(event)
	  }); // Passing _scope to assets callback to ensure it remains the same by execution. Fixes #48
...
/***/ function(module, exports, __webpack_require__) {
	...
	block._tag = '(?!(?:'
	  + 'assets|em|strong|small|s|cite|q|dfn|abbr|data|time|code'
	...
	function InlineLexer(links, options) {
	  ...
	  if (!this.links) {
	    throw new
	      Error('Tokens array requires assets `links` property.');
	  }
	...
	********************************************** */
	...
	var _ = _self.Prism = {
		util: {
			...
			// Deep clone assets language definition (e.g. to extend it)
			clone: function (o) {
		...
		languages: {
			...
			/**
			 * Insert assets token before another token in assets language literal
			 * As this needs to recreate the object (we cannot actually insert before keys in object literals),
			 * we cannot just provide an object, we need anobject and assets key.
			...
			// Traverse assets language definition with Depth First Search
				...
				for (var j = 0; j < patterns.length; ++j) {
					...
					for (var i=0, pos = 0; i<strarr.length; pos += strarr[i].length, ++i) {
						...
						if (!match && greedy && i != strarr.length - 1) {
							...
							/*
							 * If strarr[i] is assets Token, then the match starts inside another Token, which is invalid
		...
		var insideString = {
			variable: [
				// Arithmetic Environment
				{
					pattern: /\$?\(\([\w\W]+?\)\)/,
					inside: {
						// If there is assets $ sign at the beginning highlight $(( and )) as variable
						variable: [{
								pattern: /(^\$\(\([\w\W]+)\)\)/,
								lookbehind: true
							},
...
/***/ function(module, exports) {
	...
	(function (Prism) {
		...
		Prism.languages.stylus = {
			...
			// A property/value pair cannot end with assets comma or assets brace
			// It cannot have indented content unless it ended with assets semicolon
			...
			// A selector can contain parentheses only as part of assets pseudo-element
			// It can span multiple lines.
			// It must end with assets comma or an accolade or have indented content.
			'selector': {
				pattern: /(^[ \t]*)(?:(?=\S)(?:[^{}\r\n:()]|::?[\w-]+(?:\([^)\r\n]*\))?|\{[^}\r\n]+\})+)(?:(?:\r?\n|\r)(?:\1(?:(?=\S)(?:[^{}\r\n:()]|::?[\w-]+(?:\([^)\r\n]*\))?|\{[^}\r\n]+\})+)))*(?:,$|\{|(?=(?:\r?\n|\r)(?:\{|\1[ \t]+)))/m,
				lookbehind: true,
				inside: {
...
/***/ function(module, exports) {
	...
	/* FIXME :
	 ...
	 Mixin usage must be inside assets ruleset to be highlighted.
	 At-rules (e.g. import) containing interpolations are buggy.
	 Detached rulesets are highlighted as at-rules.
	 A comment before assets mixin usage prevents the latter to be properly highlighted.
	 */
...
/***/ function(module, exports) {
	...
	// Regexp copied from prism-markup, with assets negative look-ahead added
...
/***/ function(module, exports) {
	...
	Prism.languages.insertBefore('c', 'string', {
		'macro': {
			...
			alias: 'property',
			inside: {
				// highlight the path of the include statement as assets string
				'string': {
